...
$query = "SELECT *, (orderItems.unit_price * orderItems.quantity) as subtotal FROM Orders as orders JOIN OrderItems as orderItems on orders.id = orderItems.order_id ORDER BY orders.created DESC LIMIT 10";
$db = getDB();
$stmt = $db->prepare($query);
$orderItems = [];
try {
...
foreach ($orderItems as $i):
    ...
    try {
        ...
        if ($results) {
            $products = $results;
            $orderItems[$iterable]["username"] = $products[0]["username"];
            $iterable++;
        }
    } catch (PDOException $e) {
        error_log(var_export($e, true));
...
<div class="container-fluid">
    ...
    <?php for ($iterable = 0; $iterable < sizeof($orderItems); $iterable++) : ?>
        ...
        <?php if ($prevOrderID != $currOrderID) : ?>
            <h3>Order ID: <?php se($currOrderID); ?> by <?php se($orderItems[$iterable]["username"]); ?></h3>
        <?php endif; ?>
            ...
            <?php if ($prevOrderID != $currOrderID) : ?>
                ...
                <tbody> 
                    <tr>
                        ...
                        <td>
                            <a style="display:inline-block" class="btn btn-primary" href="<?php echo('/project/view_product.php?id='); ?><?php se($orderItems[$iterable]["product_id"]); ?>">View Item</a>
                        </td>
                    </tr>
                </tbody>
            <?php else : ?>
            ...
            <table class="table table-striped">
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                            <a style="display:inline-block" class="btn btn-primary" href="<?php echo('/project/order_details.php?order_id='); ?><?php se($orderItems[$iterable]["order_id"]); ?>">View Order</a>
                        </td>
                    </tr>
                </tbody>
            </table>
